...
class ExtraTask:
        ...
        try:
            ...
            for task in extra_tasks:
                ...
                try:
                    ...
                    success_count += 1
                    self.logger.info(
                        f"成功完成第 {success_count}/{remaining_tasks} 个额外评分任务"
                    )
                    ...
                    self.logger.warning(
                        f"处理歌曲 {task['work']['name']} 失败，尝试下一个: {str(e)}"
                    )
                    continue
            ...
            if success_count < remaining_tasks:
                self.logger.warning(
                    f"未能完成所有额外评分任务，仅完成 {success_count}/{remaining_tasks} 个"
                )
    ...
    def _get_extra_tasks(self) -> Tuple[List[Dict], int]:
        ...
        try:
            ...
            if response["code"] != 200:
                raise RuntimeError(
                    f"获取额外任务失败: {response.get('message', '未知错误')}"
                )
    ...
    def _report_listen(self, work: Dict) -> None:
        ...
        try:
            ...
            if response["code"] != 200:
                raise RuntimeError(
                    f"上报听歌记录失败: {response.get('message', '未知错误')}"
                )
            else:
