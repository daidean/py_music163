...
class Signer:
    def __init__(self, session: Session, task_id: str, logger: Logger):
        ...
        self.logger = logger
        self.sign_url = (
            "https://interface.music.163.com/weapi/music/partner/work/evaluate"
        )
    ...
    def sign(self, work: dict, is_extra: bool = False) -> None:
        ...
        try:
            ...
            if response["code"] == 200:
                self.logger.info(
                    f"{work['name']}「{work['authorName']}」评分完成：{score}分"
                )
            else:
                ...
                if "频繁" in error_msg:
                    ...
                    raise RuntimeError(
                        f"评分失败: {error_msg} (响应码: {response.get('code')})"
                    )
